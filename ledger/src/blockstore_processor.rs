...
fn process_entries(
    ...
    let leader = leader_schedule_cache
        .and_then(|cache| cache.slot_leader_at(slot, Some(bank)));
    ...
    } in entries
    {
        match entry {
            EntryType::Tick(hash) => {
                ...
                let tick_index = ((bank.tick_height() + tick_hashes.len() as u64) % ticks_per_slot) as u64;
                ...
                if !skip_tick_notifications {
                    if let Some(ref tick_notifier) = tick_notifier {
                        ...
                        tick_notifier.as_ref()(slot, tick_index, &poh_entry, leader.as_ref(), TICK_SOURCE_BLOCKSTORE_PROCESSOR);
                    }
                }
...
fn confirm_slot_entries(
    ...
    let num_txs = entries
        ...
        .map(|(i, entry)| {
            ...
            if entry.is_tick() {
                if let Some(ref tick_notifier) = tick_notifier {
                    let tick_index = ((bank.tick_height() + ticks_seen_in_batch) % ticks_per_slot) as u64;
                    let poh_entry = PohEntry {
                        num_hashes: hashes_per_tick,
                        hash: entry.hash,
                    };
