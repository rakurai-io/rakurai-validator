...
impl BroadcastStage {
    ...
    fn new(
        ...
        // Not constrained by --bind-address, so the OS routing table selects the appropriate
...
pub fn broadcast_shreds(
    ...
    // All external receivers (shredstream + ShredReceiverAddresses) use the dedicated
    // shred_receiver_socket bound to 0.0.0.0 so the OS routing table selects
    // the correct interface even when XDP is active on --bind-address.
    ...
    match socket {
        ...
        BroadcastSocket::Xdp(s) => {
            ...
            // External receivers must use the kernel IP stack even when XDP is active.
            if !external_packets.is_empty() {
                match batch_send(shred_receiver_socket, external_packets.iter().copied()) {
                    Ok(()) => (),
                    Err(SendPktsError::IoError(ioerr, num_failed)) => {
