...
2. **Python `tx_log_analyzer.py` Script**
...
```bash
python3 tx_log_analyzer.py /local/path/for/analysis/tx_io.log*
```
...
You may replace `tx_log_analyzer.py` with your own script, as long as it compares `tx_in` and `tx_out` logs in a similar multiset-aware manner.
