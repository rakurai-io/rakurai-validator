use {
    ...
    itertools::Itertools,
    log::{error, info, warn},
    reward_distribution::state::{
        ClaimStatus, RewardCollectionAccount, RewardDistributionConfigAccount,
    },
    ...
    for tree in &merkle_trees.generated_merkle_trees {
        ...
        for node in &tree.tree_nodes {
            ...
            let instruction = Instruction {
                program_id: reward_distribution_program_id,
                data: reward_distribution::instruction::Claim {
                    proof: node.proof.clone().unwrap(),
                    amount: node.amount,
                    bump: node.claim_status_bump,
                }
                .data(),
                accounts: reward_distribution::accounts::Claim {
                    config: reward_distribution_config,
                    reward_collection_account: tree.reward_distribution_account,
